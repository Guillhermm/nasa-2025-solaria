...
const NASAImageViewer: React.FC = () => {
  ...
  const [currentResolutionIndex, setCurrentResolutionIndex] =
    useState<number>(0);
  const [flags, setFlags] = useState<Flag[]>([]);
  const [showFlagModal, setShowFlagModal] = useState<boolean>(false);
  const [flagModalPosition, setFlagModalPosition] = useState<Position>({
    x: 0,
    y: 0,
  });
  ...
  const updateMinimapViewport = useCallback(() => {
    ...
    const visiblePortionHeight = Math.min(
      visibleHeight / displayedImageHeight,
      1
    );
    ...
    const visibleY =
      (-position.y / scale - imageOffsetY) / displayedImageHeight;

    // Convert to minimap coordinates
    const viewportWidth = Math.max(visiblePortionWidth * minimapRect.width, 10);
    const viewportHeight = Math.max(
      visiblePortionHeight * minimapRect.height,
      10
    );

    const viewportX = Math.max(
      0,
      Math.min(visibleX * minimapRect.width, minimapRect.width - viewportWidth)
    );
    const viewportY = Math.max(
      0,
      Math.min(
        visibleY * minimapRect.height,
        minimapRect.height - viewportHeight
      )
    );
  ...
  const loadImageResolution = useCallback(
  ...
  const getImagePositionFromClick = useCallback(
    (clientX: number, clientY: number): Position => {
      ...
      const percentX =
        ((transformedX - imageOffsetX) / displayedImageWidth) * 100;
      const percentY =
        ((transformedY - imageOffsetY) / displayedImageHeight) * 100;
  ...
  const clearAllFlags = useCallback(() => {
    if (
      window.confirm(
        "Are you sure you want to delete all flags? This action cannot be undone."
      )
    ) {
      setFlags([]);
      setSelectedFlagId(null);
  ...
  const setZoom = useCallback(
    (zoomPercent: number, clientX?: number, clientY?: number) => {
  ...
  const zoom = useCallback(
    (direction: number, clientX: number, clientY: number) => {
  ...
  const handleContextMenu = useCallback(
    (e: React.MouseEvent) => {
      ...
      if (
        ...
        target.classList.contains("flag-label")
      ) {
        return;
      }
  ...
  const handleMinimapClick = useCallback((e: React.MouseEvent) => {
    if (e.target !== minimapImageRef.current || isMinimapDraggingRef.current)
      return;
  ...
  const updateMainViewFromMinimap = useCallback(
  ...
  useEffect(() => {
    const handleMinimapMouseMove = (e: MouseEvent) => {
      ...
      newX = Math.max(
        0,
        Math.min(newX, minimapRect.width - viewportRect.width)
      );
      newY = Math.max(
        0,
        Math.min(newY, minimapRect.height - viewportRect.height)
      );
    ...
    <div className="nasa-image-viewer">
      <header>
        ...
        <p className="subtitle">
          Explore massive NASA images and add annotations with flags
        </p>
      </header>
      ...
      <div className="container">
          ...
          <div ref={imageContainerRef} className="image-container">
            ...
            {flags.map((flag) => (
              <div
                key={flag.id}
                className={`flag ${
                  selectedFlagId === flag.id ? "flag-selected" : ""
                }`}
            ...
            {flags.map((flag) => (
              <div
                ...
                className={`minimap-flag ${
                  selectedFlagId === flag.id ? "minimap-flag-selected" : ""
                }`}
                style={{ left: `${flag.x}%`, top: `${flag.y}%` }}
              />
            ))}
        ...
        <div className="controls">
          <div className="zoom-controls">
            ...
            <button id="reset" onClick={resetView}>
              Reset
            </button>
          </div>
          <div className="resolution-info">
            Resolution: {IMAGE_RESOLUTIONS[currentResolutionIndex].name} (
            {IMAGE_RESOLUTIONS[currentResolutionIndex].width}px)
          </div>
        </div>
        {/* Flags Table */}
              ...
              <tbody>
                {flags.map((flag) => (
                  <tr
                    key={`table-${flag.id}`}
                    className={
                      selectedFlagId === flag.id ? "table-row-selected" : ""
                    }
                ...
                {flags.length === 0 && (
                  <tr>
                    <td colSpan={4} className="no-flags-message">
                      No flags added yet. Right-click on the image to add a
                      flag.
                    </td>
                  </tr>
                )}
        ...
        <div className="flag-modal">
          ...
          <div className="flag-modal-buttons">
            ...
            <button
              ...
              onClick={editingFlag ? handleRenameFlag : handleAddFlag}
            >
